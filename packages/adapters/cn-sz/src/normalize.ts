...
function alignTimes(first: string[] | undefined, last: string[] | undefined): {
  first: string[];
  last: string[];
} {
...
export function normalize(input: SuzhouRawInput): SuzhouCanonical {
  ...
  for (const seq of emitSeqs) {
    ...
    for (let i = 0; i < patternStopIds.length - 1; i++) {
      ...
      const plannedEdge =
        (stops.find((s) => s.id === aStop)?.extras as
          | { planned_extension?: boolean }
          | undefined
        )?.planned_extension === true ||
        (stops.find((s) => s.id === bStop)?.extras as
          | { planned_extension?: boolean }
          | undefined
        )?.planned_extension === true;
      segments.push({
  ...
  for (const meta of extensionMetas) {
    ...
    const connectorStopId =
      stopByLineMapId.get(`${parentLineId}|${connectorMapId}`) ??
      stopIdByMapId.get(connectorMapId);
    ...
    const seqStationIds = attachAtEnd
      ? [
          stops.find((s) => s.id === connectorStopId)?.station_id ?? '',
          ...extStationIds
        ]
      : [
          ...extStationIds,
  ...
  for (const phys of physByName.values()) {
    ...
    const displayLines = stopLinesByStation.get(phys.id) ?? [...phys.lineNos].filter((n) => !n.includes('延线'));
