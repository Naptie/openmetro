/**
 * Typecheck every discovered adapter package.
...
async function main() {
  ...
  console.log(`typechecking ${adapters.length} adapter(s)`);
  const { spawn } = await import('node:child_process');
  for (const a of adapters) {
    const tsconfig = join(a.dir, 'tsconfig.json');
    await new Promise<void>((resolvePromise, reject) => {
      const child = spawn('bunx', ['tsc', '-p', tsconfig, '--noEmit'], {
        cwd: ROOT,
        stdio: 'inherit',
        shell: true
      });
      child.on('exit', (code) =>
        code === 0 ? resolvePromise() : reject(new Error(`${a.name} typecheck exited ${code}`))
      );
    });
  }
}
