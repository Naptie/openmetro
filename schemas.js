...
var apiLineStatusSchema = z.enum(["operating", "under_construction"]);
var apiLineSchema = z.object({ "id": z.string(), "name": z.string(), "names": apiNamesSchema, "color": z.string().optional(), "text_color": z.string().optional(), "short_name": z.string(), "mode": z.enum(["metro", "suburban_rail", "light_rail", "tram", "monorail", "airport_express", "other"]), "status": z.enum(["operating", "under_construction"]), "loop": z.boolean() });
var apiLineListSchema = z.array(apiLineSchema);
var apiStationStatusSchema = z.enum(["operating", "out_of_service", "under_construction"]);
var apiStationSchema = z.object({ "id": z.string(), "name": z.string(), "names": apiNamesSchema, "location": apiGeoPointSchema.optional(), "schematic": apiSchematicPointSchema.optional(), "status": z.enum(["operating", "out_of_service", "under_construction"]), "lines": z.array(z.string()), "is_interchange": z.boolean() });
...
var apiTravelTimeSourceSchema = z.enum(["source", "planner", "last_train", "estimated"]);
...
var apiLayerQualitySchema = z.object({ "precision": z.enum(["official", "derived", "default"]), "coverage": z.number(), "status": z.enum(["complete", "partial", "derived", "unavailable"]), "counts": z.record(z.string(), z.number()) });
var apiNetworkQualitySchema = z.object({ "topology": apiLayerQualitySchema, "coordinates": apiLayerQualitySchema, "names": apiLayerQualitySchema, "segment_times": apiLayerQualitySchema, "segment_distances": apiLayerQualitySchema, "transfer_times": apiLayerQualitySchema, "timetables": apiLayerQualitySchema, "schematic": apiLayerQualitySchema, "fares": apiLayerQualitySchema.optional() });
var apiNetworkSchema = z.object({ "id": z.string(), "name": z.string(), "names": apiNamesSchema, "city": apiCitySchema, "country_code": z.string(), "currency": z.string(), "timezone": z.string(), "coordinate_system": z.enum(["wgs84", "gcj02", "bd09", "schematic", "none"]), "default_units": z.object({ "distance": z.string(), "time": z.string(), "speed": z.string() }), "routing": apiRoutingDefaultsSchema, "quality": apiNetworkQualitySchema.optional(), "synced_at": z.string().optional() });
